...
app.config['SESSION_COOKIE_NAME'] = 'bts_dlc_session'
...
def _build_sso_token(username: str, role: str) -> str:
    ...
    payload = {"u": username, "r": role, "ts": int(datetime.now().timestamp())}
...
def _ui_patient_url() -> str:
    base = _derive_ui_base_url()
    try:
        username = current_user.username
        role = current_user.role
        token = _build_sso_token(username, role)
        return f"{base}/sso?token={token}"
    except Exception:
        return f"{base}/patient_portal"
    ...
    try:
        ...
        if diagnosis:
            ...
            if current_user.role == 'admin':
                ...
                flash('权限不足！只有管理员可以删除诊断记录。', 'danger')
...
def add_diagnosis(patient_id):
    if current_user.role != 'admin':
        flash('权限不足！只有管理员可以添加诊断记录。', 'danger')
        return redirect(url_for('diagnoses', patient_id=patient_id))
    # 获取当前日期
