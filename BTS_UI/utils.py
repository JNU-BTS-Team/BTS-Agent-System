...
def resolve_model_provider(model_info):
    if model_info == 'gemini-pro':
        return 'gemini', model_info
    if model_info in OPENAI_MODEL_MAP:
        return 'openai', OPENAI_MODEL_MAP[model_info]
    if model_info.startswith('gpt-'):
        return 'openai', model_info
    if model_info.startswith('deepseek-'):
        return 'deepseek', model_info
    raise ValueError(f"Unsupported model: {model_info}")


def get_required_api_key_env(model_info):
    provider, _ = resolve_model_provider(model_info)
    if provider == 'gemini':
...
def create_openai_compatible_client(model_info):
    provider, resolved_model = resolve_model_provider(model_info)
    api_key_env = get_required_api_key_env(model_info)
    api_key = os.environ.get(api_key_env) or os.environ.get(api_key_env.upper())
    if not api_key:
        raise EnvironmentError(f"缺少 {api_key_env}/{api_key_env.upper()} 环境变量。")
...
class Agent:
    def __init__(self, instruction, role, examplers=None, model_info='gpt-4o-mini', img_path=None):
        ...
        self.provider, self.api_model_name = resolve_model_provider(model_info)

        if self.provider == 'gemini':
            if genai is None:
                raise EnvironmentError("缺少 google-generativeai 依赖，无法使用 gemini 模型。")
            self.model = genai.GenerativeModel('gemini-pro')
            self._chat = self.model.start_chat(history=[])
        else:
            self.client, self.api_model_name = create_openai_compatible_client(model_info)
            self.messages = [
    ...
    def chat(self, message, img_path=None, chat_mode=True):
        if self.provider == 'gemini':
            if genai is None:
                return "Error: missing google-generativeai dependency."
            for _ in range(10):
                try:
...
def setup_model(model_name):
    provider, _ = resolve_model_provider(model_name)
    if provider == 'gemini':
        genai.configure(api_key=os.environ['genai_api_key'])
        return genai, None
    elif provider in ['openai', 'deepseek']:
        client, _ = create_openai_compatible_client(model_name)
        return None, client
    else:
