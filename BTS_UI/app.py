...
app.config['MYSQL_PASSWORD'] = 'Hjk608866'
...
def _get_remote_hosts():
    ...
    if host:
        return [host]
    return ["10.125.125.2", "117.50.179.58"]
...
def _connect_remote_ssh():
    ...
    timeout = int(os.environ.get("REMOTE_SSH_TIMEOUT", "15"))
    banner_timeout = int(os.environ.get("REMOTE_SSH_BANNER_TIMEOUT", "60"))
    auth_timeout = int(os.environ.get("REMOTE_SSH_AUTH_TIMEOUT", "45"))
...
def login():
    if request.method == 'POST':
        ...
        if user and user['password'] == hash_password(password):
            ...
            login_user(user_obj)
            flash('登录成功！', 'success')
            login_to_dlc = (os.environ.get('BTS_LOGIN_TO_DLC') or '1').strip().lower() in {'1', 'true', 'yes', 'y', 'on'}
            if login_to_dlc:
...
def patient_portal():
    ...
    dlc_base = _derive_dlc_base_url()
    entry_url = f"{dlc_base}/main_interface"
    patient_portal_url = url_for('patient_portal')
    patients_url = url_for('patients')
    doctors_url = f"{dlc_base}/team_management"
    test1_url = f"{dlc_base}/main_interface"
...
def api_doctors():
    ...
    if not username or not password:
        return jsonify(success=False, message="missing_username_or_password"), 400
